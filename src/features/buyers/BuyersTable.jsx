...
export default function BuyersTable({
	...
	const cols = [
		...
		{ key: 'registration_type', label: 'Registration' },
					...
					<tbody className="divide-y divide-gray-100 text-sm">
						{rows.map(row => (
							<tr key={row.id} className="hover:bg-gray-50">
								<td className="px-4 py-3 text-black">{row.business_name}</td>
								<td className="px-4 py-3">{row.ntn_cnic || '—'}</td>
								<td className="px-4 py-3">{row.province || '—'}</td>
								<td className="px-4 py-3 capitalize">{row.registration_type || '—'}</td>
						...
						{rows.length === 0 && (
							<tr><td colSpan={cols.length} className="px-4 py-6 text-center text-gray-600">No buyers found.</td></tr>
						)}
					</tbody>
				</table>
			</div>
